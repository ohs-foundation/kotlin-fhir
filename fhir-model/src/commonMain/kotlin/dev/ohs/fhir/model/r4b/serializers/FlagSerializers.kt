...
internal object FlagSerializer : KSerializer<Flag> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Flag {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        17 ->
          period = decoder.decodeNullableSerializableElement(__desc, 17, Hoisted.periodSer, null)
        18 ->
          encounter =
            decoder.decodeNullableSerializableElement(__desc, 18, Hoisted.subjectSer, null)
        19 ->
          author = decoder.decodeNullableSerializableElement(__desc, 19, Hoisted.subjectSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else -> throw SerializationException("Unexpected index decoding Flag: " + __i)
      }
    }
    return Flag(
      id = id,
  ...
  internal fun serializeJson(encoder: CompositeEncoder, `value`: Flag) {
    val __desc = descriptor
    ...
    (value.period)?.let { encoder.encodeSerializableElement(__desc, 17, Hoisted.periodSer, it) }
    (value.encounter)?.let { encoder.encodeSerializableElement(__desc, 18, Hoisted.subjectSer, it) }
    (value.author)?.let { encoder.encodeSerializableElement(__desc, 19, Hoisted.subjectSer, it) }
  }
...
internal object FlagPolymorphicSerializer : KSerializer<Flag> {
  ...
  override fun serialize(encoder: Encoder, `value`: Flag) {
    encoder.encodeStructure(descriptor) { FlagSerializer.serializeJson(this, value) }
  }

  override fun deserialize(decoder: Decoder): Flag =
    decoder.decodeStructure(descriptor) { FlagSerializer.deserializeJson(this) }
}
