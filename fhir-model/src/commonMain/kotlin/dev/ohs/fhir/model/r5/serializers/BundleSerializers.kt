...
internal object BundleSerializer : KSerializer<Bundle> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Bundle") {
      ...
      element("issues", Resource.serializer().descriptor, isOptional = true)
    }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Bundle {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        17 ->
          issues = decoder.decodeNullableSerializableElement(__desc, 17, Hoisted.issuesSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: Bundle) {
    ...
    (value.issues)?.let { encoder.encodeSerializableElement(__desc, 17, Hoisted.issuesSer, it) }
  }
