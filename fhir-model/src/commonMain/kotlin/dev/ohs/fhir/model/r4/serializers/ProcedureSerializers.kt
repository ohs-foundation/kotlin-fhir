...
internal object ProcedurePerformedSerializer : KSerializer<Procedure.Performed> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Procedure.Performed") {
    ...
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        ...
        is Procedure.Performed.Age -> {
          encodeSerializableElement(__desc, 5, Hoisted.performedAgeSer, __d.value)
        }
        is Procedure.Performed.Range -> {
          encodeSerializableElement(__desc, 6, Hoisted.performedRangeSer, __d.value)
        }
      }
    }
  }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Procedure.Performed {
    val __desc = descriptor
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        0 -> performedDateTime = decoder.decodeStringElement(__desc, 0)
        1 ->
          _performedDateTime =
        ...
        5 ->
          performedAge =
            decoder.decodeNullableSerializableElement(__desc, 5, Hoisted.performedAgeSer, null)
        6 ->
          performedRange =
            decoder.decodeNullableSerializableElement(__desc, 6, Hoisted.performedRangeSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else ->
          throw SerializationException("Unexpected index decoding Procedure.Performed: " + __i)
      }
    }
    return Procedure.Performed.from(
