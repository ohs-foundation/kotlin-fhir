...
internal object RequestGroupActionRelatedActionOffsetSerializer :
  KSerializer<RequestGroup.Action.RelatedAction.Offset> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("RequestGroup.Action.RelatedAction.Offset") {
      element("offsetDuration", Duration.serializer().descriptor, isOptional = true)
      element("offsetRange", Range.serializer().descriptor, isOptional = true)
    }
      ...
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding RequestGroup.Action.RelatedAction.Offset: " + __i
          )
      }
    }
    return RequestGroup.Action.RelatedAction.Offset.from(offsetDuration, offsetRange)!!
  }
...
internal object RequestGroupActionTimingSerializer : KSerializer<RequestGroup.Action.Timing> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("RequestGroup.Action.Timing") {
      element("timingDateTime", KotlinString.serializer().descriptor, isOptional = true)
      element("_timingDateTime", Element.serializer().descriptor, isOptional = true)
      element("timingAge", Age.serializer().descriptor, isOptional = true)
      element("timingPeriod", Period.serializer().descriptor, isOptional = true)
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): RequestGroup.Action.Timing {
    val __desc = descriptor
    var timingDateTime: KotlinString? = null
    var _timingDateTime: Element? = null
    var timingAge: Age? = null
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding RequestGroup.Action.Timing: " + __i
          )
      }
    }
    return RequestGroup.Action.Timing.from(
      DateTime.of(FhirDateTime.fromString(timingDateTime), _timingDateTime),
      timingAge,
      timingPeriod,
      timingDuration,
