  ...
  private fun deserializeJson(decoder: CompositeDecoder): ClinicalImpression.Finding {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        4 -> basis = decoder.decodeStringElement(__desc, 4)
        5 -> _basis = decoder.decodeNullableSerializableElement(__desc, 5, Hoisted.basisSer, null)
...
internal object ClinicalImpressionSerializer : KSerializer<ClinicalImpression> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): ClinicalImpression {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        25 ->
          previous = decoder.decodeNullableSerializableElement(__desc, 25, Hoisted.subjectSer, null)
        26 ->
          problem = decoder.decodeNullableSerializableElement(__desc, 26, Hoisted.problemSer, null)
        27 ->
          changePattern =
            decoder.decodeNullableSerializableElement(__desc, 27, Hoisted.statusReasonSer, null)
        28 ->
          protocol =
            decoder.decodeNullableSerializableElement(__desc, 28, Hoisted.protocolSer, null)
        29 ->
          _protocol =
            decoder.decodeNullableSerializableElement(__desc, 29, Hoisted.protocolSer2, null)
        30 -> summary = decoder.decodeStringElement(__desc, 30)
        31 ->
          _summary =
            decoder.decodeNullableSerializableElement(__desc, 31, Hoisted.implicitRulesSer, null)
        32 ->
          finding = decoder.decodeNullableSerializableElement(__desc, 32, Hoisted.findingSer, null)
        33 ->
          prognosisCodeableConcept =
        ...
        34 ->
          prognosisReference =
            decoder.decodeNullableSerializableElement(__desc, 34, Hoisted.problemSer, null)
        35 ->
          supportingInfo =
  ...
  internal fun serializeJson(encoder: CompositeEncoder, `value`: ClinicalImpression) {
    val __desc = descriptor
    ...
    (value.previous)?.let { encoder.encodeSerializableElement(__desc, 25, Hoisted.subjectSer, it) }
    if (value.problem.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 26, Hoisted.problemSer, value.problem)
    ...
    (value.protocol.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 28, Hoisted.protocolSer, it)
    }
    (value.protocol.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 29, Hoisted.protocolSer2, it)
    }
    ((value.summary?.value))?.let { encoder.encodeStringElement(__desc, 30, it) }
    ...
    if (value.finding.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 32, Hoisted.findingSer, value.finding)
    ...
    if (value.prognosisReference.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 34, Hoisted.problemSer, value.prognosisReference)
    if (value.supportingInfo.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 35, Hoisted.problemSer, value.supportingInfo)
...
internal object ClinicalImpressionPolymorphicSerializer : KSerializer<ClinicalImpression> {
  ...
  override fun serialize(encoder: Encoder, `value`: ClinicalImpression) {
    encoder.encodeStructure(descriptor) { ClinicalImpressionSerializer.serializeJson(this, value) }
  }

  override fun deserialize(decoder: Decoder): ClinicalImpression =
    decoder.decodeStructure(descriptor) { ClinicalImpressionSerializer.deserializeJson(this) }
}
