...
internal object CommunicationRequestPayloadContentSerializer :
  ...
  override fun serialize(encoder: Encoder, `value`: CommunicationRequest.Payload.Content) {
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        ...
        is CommunicationRequest.Payload.Content.Attachment -> {
          encodeSerializableElement(__desc, 2, Hoisted.contentAttachmentSer, __d.value)
        }
        is CommunicationRequest.Payload.Content.Reference -> {
          encodeSerializableElement(__desc, 3, Hoisted.contentReferenceSer, __d.value)
        }
      }
    }
  }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): CommunicationRequest.Payload.Content {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        0 -> contentString = decoder.decodeStringElement(__desc, 0)
        1 ->
          _contentString =
            decoder.decodeNullableSerializableElement(__desc, 1, Hoisted.elementSer, null)
        2 ->
          contentAttachment =
            decoder.decodeNullableSerializableElement(__desc, 2, Hoisted.contentAttachmentSer, null)
        3 ->
          contentReference =
            decoder.decodeNullableSerializableElement(__desc, 3, Hoisted.contentReferenceSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else ->
