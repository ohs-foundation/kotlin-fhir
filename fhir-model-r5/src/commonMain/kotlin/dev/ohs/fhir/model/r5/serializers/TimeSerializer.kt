...
internal object TimeSerializer : KSerializer<Time> {
  ...
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Time") {
      ...
      element("value", LocalTimeSerializer.descriptor)
    }
  ...
  override fun serialize(encoder: Encoder, `value`: Time) {
    encoder.encodeStructure(descriptor) {
      ...
      encodeNullableSerializableElement(descriptor, 2, LocalTimeSerializer, value.`value`)
    }
  }
    ...
    decoder.decodeStructure(descriptor) {
      ...
      while (true) {
        when (val index = decodeElementIndex(descriptor)) {
          ...
          2 -> `value` = decodeNullableSerializableElement(descriptor, 2, LocalTimeSerializer)
