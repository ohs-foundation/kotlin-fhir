...
internal object GuidanceResponseModuleSerializer : KSerializer<GuidanceResponse.Module> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("GuidanceResponse.Module") {
    ...
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        ...
        is GuidanceResponse.Module.CodeableConcept -> {
          encodeSerializableElement(__desc, 4, Hoisted.moduleCodeableConceptSer, __d.value)
        }
      }
    }
  }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): GuidanceResponse.Module {
    val __desc = descriptor
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        0 -> moduleUri = decoder.decodeStringElement(__desc, 0)
        1 ->
          _moduleUri =
            decoder.decodeNullableSerializableElement(__desc, 1, Hoisted.elementSer, null)
        2 -> moduleCanonical = decoder.decodeStringElement(__desc, 2)
        3 ->
          _moduleCanonical =
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException("Unexpected index decoding GuidanceResponse.Module: " + __i)
      }
    }
    return GuidanceResponse.Module.from(
...
internal object GuidanceResponseSerializer : KSerializer<GuidanceResponse> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): GuidanceResponse {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        25 ->
          reason = decoder.decodeNullableSerializableElement(__desc, 25, Hoisted.reasonSer, null)
        ...
        29 ->
          result = decoder.decodeNullableSerializableElement(__desc, 29, Hoisted.resultSer, null)
        30 ->
          dataRequirement =
            decoder.decodeNullableSerializableElement(__desc, 30, Hoisted.dataRequirementSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: GuidanceResponse) {
    ...
    if (value.reason.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 25, Hoisted.reasonSer, value.reason)
    ...
    (value.outputParameters)?.let {
      encoder.encodeSerializableElement(__desc, 28, Hoisted.subjectSer, it)
    }
    if (value.result.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 29, Hoisted.resultSer, value.result)
