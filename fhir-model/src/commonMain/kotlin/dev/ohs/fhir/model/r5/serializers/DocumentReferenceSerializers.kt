...
internal object DocumentReferenceContentProfileValueSerializer :
  KSerializer<DocumentReference.Content.Profile.Value> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("DocumentReference.Content.Profile.Value") {
      element("valueCoding", Coding.serializer().descriptor, isOptional = true)
      element("valueUri", KotlinString.serializer().descriptor, isOptional = true)
      element("_valueUri", Element.serializer().descriptor, isOptional = true)
      element("valueCanonical", KotlinString.serializer().descriptor, isOptional = true)
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): DocumentReference.Content.Profile.Value {
    val __desc = descriptor
    var valueCoding: Coding? = null
    var valueUri: KotlinString? = null
    var _valueUri: Element? = null
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        1 -> valueUri = decoder.decodeStringElement(__desc, 1)
        2 ->
          _valueUri = decoder.decodeNullableSerializableElement(__desc, 2, Hoisted.elementSer, null)
        3 -> valueCanonical = decoder.decodeStringElement(__desc, 3)
        4 ->
          _valueCanonical =
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding DocumentReference.Content.Profile.Value: " + __i
          )
      }
    }
    return DocumentReference.Content.Profile.Value.from(
      valueCoding,
      Uri.of(valueUri, _valueUri),
      Canonical.of(valueCanonical, _valueCanonical),
    )!!
...
internal object DocumentReferenceSerializer : KSerializer<DocumentReference> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("DocumentReference") {
      ...
      element("context", listSerialDescriptor(Reference.serializer().descriptor), isOptional = true)
      element(
        "event",
