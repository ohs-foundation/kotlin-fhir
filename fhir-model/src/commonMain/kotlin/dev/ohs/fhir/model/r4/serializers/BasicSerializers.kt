...
internal object BasicSerializer : KSerializer<Basic> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Basic {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        13 ->
          subject = decoder.decodeNullableSerializableElement(__desc, 13, Hoisted.subjectSer, null)
        14 -> created = decoder.decodeStringElement(__desc, 14)
        15 ->
          _created =
            decoder.decodeNullableSerializableElement(__desc, 15, Hoisted.implicitRulesSer, null)
        16 ->
          author = decoder.decodeNullableSerializableElement(__desc, 16, Hoisted.subjectSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else -> throw SerializationException("Unexpected index decoding Basic: " + __i)
      }
    }
    return Basic(
      id = id,
      ...
      code = code!!,
      subject = subject,
      created = Date.of(FhirDate.fromString(created), _created),
      author = author,
    )
  }

  internal fun serializeJson(encoder: CompositeEncoder, `value`: Basic) {
    val __desc = descriptor
    ...
    (value.subject)?.let { encoder.encodeSerializableElement(__desc, 13, Hoisted.subjectSer, it) }
    ((value.created?.value?.toString()))?.let { encoder.encodeStringElement(__desc, 14, it) }
    (value.created?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 15, Hoisted.implicitRulesSer, it)
    }
    (value.author)?.let { encoder.encodeSerializableElement(__desc, 16, Hoisted.subjectSer, it) }
  }
...
internal object BasicPolymorphicSerializer : KSerializer<Basic> {
  ...
  override fun serialize(encoder: Encoder, `value`: Basic) {
    encoder.encodeStructure(descriptor) { BasicSerializer.serializeJson(this, value) }
  }

  override fun deserialize(decoder: Decoder): Basic =
    decoder.decodeStructure(descriptor) { BasicSerializer.deserializeJson(this) }
}
