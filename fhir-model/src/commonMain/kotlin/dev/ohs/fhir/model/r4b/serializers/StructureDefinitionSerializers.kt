...
internal object StructureDefinitionSerializer : KSerializer<StructureDefinition> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): StructureDefinition {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        37 ->
          keyword = decoder.decodeNullableSerializableElement(__desc, 37, Hoisted.keywordSer, null)
        38 -> fhirVersion = decoder.decodeStringElement(__desc, 38)
        39 ->
          _fhirVersion =
            decoder.decodeNullableSerializableElement(__desc, 39, Hoisted.implicitRulesSer, null)
        40 ->
          mapping = decoder.decodeNullableSerializableElement(__desc, 40, Hoisted.mappingSer, null)
        41 -> kind = decoder.decodeStringElement(__desc, 41)
        42 ->
          _kind =
            decoder.decodeNullableSerializableElement(__desc, 42, Hoisted.implicitRulesSer, null)
        43 -> `abstract` = decoder.decodeBooleanElement(__desc, 43)
        44 ->
          _abstract =
            decoder.decodeNullableSerializableElement(__desc, 44, Hoisted.implicitRulesSer, null)
        45 ->
          context = decoder.decodeNullableSerializableElement(__desc, 45, Hoisted.contextSer, null)
        46 ->
          contextInvariant =
            decoder.decodeNullableSerializableElement(__desc, 46, Hoisted.contextInvariantSer, null)
        47 ->
          _contextInvariant =
        ...
        50 -> baseDefinition = decoder.decodeStringElement(__desc, 50)
        51 ->
          _baseDefinition =
            decoder.decodeNullableSerializableElement(__desc, 51, Hoisted.implicitRulesSer, null)
        52 -> derivation = decoder.decodeStringElement(__desc, 52)
        53 ->
          _derivation =
            decoder.decodeNullableSerializableElement(__desc, 53, Hoisted.implicitRulesSer, null)
        54 ->
          snapshot =
            decoder.decodeNullableSerializableElement(__desc, 54, Hoisted.snapshotSer, null)
        55 ->
          differential =
            decoder.decodeNullableSerializableElement(__desc, 55, Hoisted.differentialSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else ->
  ...
  internal fun serializeJson(encoder: CompositeEncoder, `value`: StructureDefinition) {
    val __desc = descriptor
    ...
    if (value.keyword.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 37, Hoisted.keywordSer, value.keyword)
    ((value.fhirVersion?.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 38, it) }
    ...
    if (value.mapping.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 40, Hoisted.mappingSer, value.mapping)
    ((value.kind.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 41, it) }
    (value.kind.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 42, Hoisted.implicitRulesSer, it)
    }
    ((value.`abstract`.value))?.let { encoder.encodeBooleanElement(__desc, 43, it) }
    ...
    if (value.context.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 45, Hoisted.contextSer, value.context)
    (value.contextInvariant.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 46, Hoisted.contextInvariantSer, it)
    }
    (value.contextInvariant.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 47, Hoisted.contextInvariantSer2, it)
    }
    ((value.type.value))?.let { encoder.encodeStringElement(__desc, 48, it) }
    (value.type.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 49, Hoisted.implicitRulesSer, it)
    }
    ((value.baseDefinition?.value))?.let { encoder.encodeStringElement(__desc, 50, it) }
    (value.baseDefinition?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 51, Hoisted.implicitRulesSer, it)
    }
    ((value.derivation?.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 52, it) }
    (value.derivation?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 53, Hoisted.implicitRulesSer, it)
    }
    (value.snapshot)?.let { encoder.encodeSerializableElement(__desc, 54, Hoisted.snapshotSer, it) }
    (value.differential)?.let {
      encoder.encodeSerializableElement(__desc, 55, Hoisted.differentialSer, it)
    }
  }
...
internal object StructureDefinitionPolymorphicSerializer : KSerializer<StructureDefinition> {
  ...
  override fun serialize(encoder: Encoder, `value`: StructureDefinition) {
    encoder.encodeStructure(descriptor) { StructureDefinitionSerializer.serializeJson(this, value) }
  }

  override fun deserialize(decoder: Decoder): StructureDefinition =
    decoder.decodeStructure(descriptor) { StructureDefinitionSerializer.deserializeJson(this) }
}
