...
internal object DiagnosticReportSerializer : KSerializer<DiagnosticReport> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("DiagnosticReport") {
      ...
      element("composition", Reference.serializer().descriptor, isOptional = true)
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): DiagnosticReport {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        29 ->
          study = decoder.decodeNullableSerializableElement(__desc, 29, Hoisted.basedOnSer, null)
        30 ->
          supportingInfo =
            decoder.decodeNullableSerializableElement(__desc, 30, Hoisted.supportingInfoSer, null)
        31 -> media = decoder.decodeNullableSerializableElement(__desc, 31, Hoisted.mediaSer, null)
        32 ->
          composition =
            decoder.decodeNullableSerializableElement(__desc, 32, Hoisted.basedOnSerInner, null)
        33 -> conclusion = decoder.decodeStringElement(__desc, 33)
        34 ->
          _conclusion =
        ...
        36 ->
          presentedForm =
            decoder.decodeNullableSerializableElement(__desc, 36, Hoisted.presentedFormSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: DiagnosticReport) {
    ...
    if (value.study.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 29, Hoisted.basedOnSer, value.study)
    if (value.supportingInfo.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 30, Hoisted.supportingInfoSer, value.supportingInfo)
    if (value.media.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 31, Hoisted.mediaSer, value.media)
    (value.composition)?.let {
      encoder.encodeSerializableElement(__desc, 32, Hoisted.basedOnSerInner, it)
    }
    ((value.conclusion?.value))?.let { encoder.encodeStringElement(__desc, 33, it) }
    ...
    if (value.conclusionCode.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 35, Hoisted.categorySer, value.conclusionCode)
    if (value.presentedForm.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 36, Hoisted.presentedFormSer, value.presentedForm)
  }
