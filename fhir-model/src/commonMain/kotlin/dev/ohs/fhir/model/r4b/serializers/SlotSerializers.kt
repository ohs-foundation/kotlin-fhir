...
internal object SlotSerializer : KSerializer<Slot> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Slot {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        13 ->
          serviceType =
            decoder.decodeNullableSerializableElement(__desc, 13, Hoisted.serviceCategorySer, null)
        14 ->
          specialty =
  ...
  internal fun serializeJson(encoder: CompositeEncoder, `value`: Slot) {
    val __desc = descriptor
    ...
    if (value.serviceType.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 13, Hoisted.serviceCategorySer, value.serviceType)
    if (value.specialty.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 14, Hoisted.serviceCategorySer, value.specialty)
    (value.appointmentType)?.let {
      encoder.encodeSerializableElement(__desc, 15, Hoisted.serviceCategorySerInner, it)
    }
...
internal object SlotPolymorphicSerializer : KSerializer<Slot> {
  ...
  override fun serialize(encoder: Encoder, `value`: Slot) {
    encoder.encodeStructure(descriptor) { SlotSerializer.serializeJson(this, value) }
  }

  override fun deserialize(decoder: Decoder): Slot =
    decoder.decodeStructure(descriptor) { SlotSerializer.deserializeJson(this) }
}
