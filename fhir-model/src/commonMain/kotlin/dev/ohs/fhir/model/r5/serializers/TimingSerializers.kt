...
internal object TimingRepeatBoundsSerializer : KSerializer<Timing.Repeat.Bounds> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Timing.Repeat.Bounds") {
      element(
        "boundsDuration",
        lazyDescriptor { Duration.serializer().descriptor },
        isOptional = true,
  ...
  override fun serialize(encoder: Encoder, `value`: Timing.Repeat.Bounds) {
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        is Timing.Repeat.Bounds.Duration -> {
          encodeSerializableElement(__desc, 0, Hoisted.boundsDurationSer, __d.value)
        }
        is Timing.Repeat.Bounds.Range -> {
          encodeSerializableElement(__desc, 1, Hoisted.boundsRangeSer, __d.value)
        }
        is Timing.Repeat.Bounds.Period -> {
          encodeSerializableElement(__desc, 2, Hoisted.boundsPeriodSer, __d.value)
        }
      }
    }
  }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Timing.Repeat.Bounds {
    val __desc = descriptor
    var boundsDuration: Duration? = null
    var boundsRange: Range? = null
    var boundsPeriod: Period? = null
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        0 ->
          boundsDuration =
            decoder.decodeNullableSerializableElement(__desc, 0, Hoisted.boundsDurationSer, null)
        1 ->
          boundsRange =
            decoder.decodeNullableSerializableElement(__desc, 1, Hoisted.boundsRangeSer, null)
        2 ->
          boundsPeriod =
            decoder.decodeNullableSerializableElement(__desc, 2, Hoisted.boundsPeriodSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else ->
          throw SerializationException("Unexpected index decoding Timing.Repeat.Bounds: " + __i)
      }
    }
    return Timing.Repeat.Bounds.from(boundsDuration, boundsRange, boundsPeriod)!!
  }
