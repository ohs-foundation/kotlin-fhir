...
internal object DeviceDefinitionManufacturerSerializer :
  KSerializer<DeviceDefinition.Manufacturer> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("DeviceDefinition.Manufacturer") {
    ...
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        ...
        is DeviceDefinition.Manufacturer.Reference -> {
          encodeSerializableElement(__desc, 2, Hoisted.manufacturerReferenceSer, __d.value)
        }
      }
    }
  }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): DeviceDefinition.Manufacturer {
    val __desc = descriptor
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        0 -> manufacturerString = decoder.decodeStringElement(__desc, 0)
        1 ->
          _manufacturerString =
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding DeviceDefinition.Manufacturer: " + __i
          )
      }
    }
    return DeviceDefinition.Manufacturer.from(
