...
internal object ContractTermActionSerializer : KSerializer<Contract.Term.Action> {
  ...
  private fun deserializeJson(decoder: CompositeDecoder): Contract.Term.Action {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        26 ->
          reason = decoder.decodeNullableSerializableElement(__desc, 26, Hoisted.reasonSer, null)
        27 ->
          reasonLinkId =
            decoder.decodeNullableSerializableElement(__desc, 27, Hoisted.linkIdSer, null)
        28 ->
          _reasonLinkId =
            decoder.decodeNullableSerializableElement(__desc, 28, Hoisted.linkIdSer2, null)
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: Contract.Term.Action) {
    ...
    if (value.reason.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 26, Hoisted.reasonSer, value.reason)
    (value.reasonLinkId.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 27, Hoisted.linkIdSer, it)
    }
    (value.reasonLinkId.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 28, Hoisted.linkIdSer2, it)
    }
    if (value.note.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 29, Hoisted.noteSer, value.note)
    (value.securityLabelNumber.map { it.value }.takeUnless { it.all { it == null } })?.let {
      encoder.encodeSerializableElement(__desc, 30, Hoisted.securityLabelNumberSer, it)
    }
    (value.securityLabelNumber.map { it.toElement() }.takeUnless { it.all { it == null } })?.let {
