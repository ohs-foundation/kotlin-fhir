...
internal object DataRequirementValueFilterSerializer : KSerializer<DataRequirement.ValueFilter> {
  ...
  private fun deserializeJson(decoder: CompositeDecoder): DataRequirement.ValueFilter {
    val __desc = descriptor
    var id: KotlinString? = null
    var extension: List<Extension>? = null
    var path: KotlinString? = null
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        5 ->
          _searchParam = decoder.decodeNullableSerializableElement(__desc, 5, Hoisted.pathSer, null)
        6 -> comparator = decoder.decodeStringElement(__desc, 6)
        7 ->
          _comparator = decoder.decodeNullableSerializableElement(__desc, 7, Hoisted.pathSer, null)
        8 -> valueDateTime = decoder.decodeStringElement(__desc, 8)
        9 ->
          _valueDateTime =
            decoder.decodeNullableSerializableElement(__desc, 9, Hoisted.pathSer, null)
        10 ->
          valuePeriod =
            decoder.decodeNullableSerializableElement(__desc, 10, Hoisted.valuePeriodSer, null)
        11 ->
          valueDuration =
            decoder.decodeNullableSerializableElement(__desc, 11, Hoisted.valueDurationSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else -> throw SerializationException("Unexpected index decoding ValueFilter: " + __i)
      }
    }
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: DataRequirement.ValueFilter) {
    val __desc = descriptor
    (value.id)?.let { encoder.encodeStringElement(__desc, 0, it) }
    if (value.extension.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 1, Hoisted.extensionSer, value.extension)
    ...
    ((value.comparator?.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 6, it) }
    ...
    when (val __d = value.`value`) {
      ...
      is DataRequirement.ValueFilter.Value.DateTime -> {
        ...
        (__d.value.toElement())?.let {
          encoder.encodeSerializableElement(__desc, 9, Hoisted.pathSer, it)
        }
      }
      is DataRequirement.ValueFilter.Value.Period -> {
        encoder.encodeSerializableElement(__desc, 10, Hoisted.valuePeriodSer, __d.value)
      }
      is DataRequirement.ValueFilter.Value.Duration -> {
        encoder.encodeSerializableElement(__desc, 11, Hoisted.valueDurationSer, __d.value)
      }
    }
  }
...
internal object DataRequirementValueFilterValueSerializer :
  KSerializer<DataRequirement.ValueFilter.Value> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("DataRequirement.ValueFilter.Value") {
      element("valueDateTime", KotlinString.serializer().descriptor, isOptional = true)
      element("_valueDateTime", Element.serializer().descriptor, isOptional = true)
      element("valuePeriod", lazyDescriptor { Period.serializer().descriptor }, isOptional = true)
      element(
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): DataRequirement.ValueFilter.Value {
    val __desc = descriptor
    var valueDateTime: KotlinString? = null
    var _valueDateTime: Element? = null
    var valuePeriod: Period? = null
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding DataRequirement.ValueFilter.Value: " + __i
          )
      }
    }
    return DataRequirement.ValueFilter.Value.from(
      DateTime.of(FhirDateTime.fromString(valueDateTime), _valueDateTime),
      valuePeriod,
      valueDuration,
    )!!
...
internal object DataRequirementSerializer : KSerializer<DataRequirement> {
  ...
  private fun deserializeJson(decoder: CompositeDecoder): DataRequirement {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        12 ->
          valueFilter =
            decoder.decodeNullableSerializableElement(__desc, 12, Hoisted.valueFilterSer, null)
        13 -> limit = decoder.decodeIntElement(__desc, 13)
        14 -> _limit = decoder.decodeNullableSerializableElement(__desc, 14, Hoisted.typeSer, null)
        15 -> sort = decoder.decodeNullableSerializableElement(__desc, 15, Hoisted.sortSer, null)
        CompositeDecoder.DECODE_DONE -> break
    ...
    return DataRequirement(
      ...
      mustSupport =
        (kotlin.collections.List(maxOf(mustSupport?.size ?: 0, _mustSupport?.size ?: 0)) { __i ->
          R5String.of(mustSupport?.getOrNull(__i)?.let { it }, _mustSupport?.getOrNull(__i))!!
        }),
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: DataRequirement) {
    ...
    if (value.valueFilter.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 12, Hoisted.valueFilterSer, value.valueFilter)
    ((value.limit?.value))?.let { encoder.encodeIntElement(__desc, 13, it) }
    (value.limit?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 14, Hoisted.typeSer, it)
    }
    if (value.sort.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 15, Hoisted.sortSer, value.sort)
  }
