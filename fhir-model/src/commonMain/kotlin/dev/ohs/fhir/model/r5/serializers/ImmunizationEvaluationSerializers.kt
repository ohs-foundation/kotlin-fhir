...
internal object ImmunizationEvaluationSerializer : KSerializer<ImmunizationEvaluation> {
    ...
    while (true) {
      ...
      when (__i - __off) {
        ...
        24 ->
          _series =
            decoder.decodeNullableSerializableElement(__desc, __i, Hoisted.implicitRulesSer, null)
        25 -> doseNumber = decoder.decodeStringElement(__desc, __i)
        26 ->
          _doseNumber =
            decoder.decodeNullableSerializableElement(__desc, __i, Hoisted.implicitRulesSer, null)
        27 -> seriesDoses = decoder.decodeStringElement(__desc, __i)
        28 ->
          _seriesDoses =
            decoder.decodeNullableSerializableElement(__desc, __i, Hoisted.implicitRulesSer, null)
        else ->
  ...
  internal fun serializeJson(
    ...
    ((value.doseNumber?.value))?.let { encoder.encodeStringElement(__desc, 25 + __off, it) }
    (value.doseNumber?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 26 + __off, Hoisted.implicitRulesSer, it)
    }
    ((value.seriesDoses?.value))?.let { encoder.encodeStringElement(__desc, 27 + __off, it) }
