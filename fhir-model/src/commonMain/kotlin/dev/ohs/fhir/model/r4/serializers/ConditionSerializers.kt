...
internal object ConditionSerializer : KSerializer<Condition> {
    ...
    while (true) {
      ...
      when (__i - __off) {
        ...
        36 ->
          asserter =
            decoder.decodeNullableSerializableElement(__desc, __i, Hoisted.subjectSer, null)
        37 -> stage = decoder.decodeNullableSerializableElement(__desc, __i, Hoisted.stageSer, null)
        38 ->
          evidence =
  ...
  internal fun serializeJson(
    ...
    (value.recorder)?.let {
      encoder.encodeSerializableElement(__desc, 35 + __off, Hoisted.subjectSer, it)
    }
    (value.asserter)?.let {
      encoder.encodeSerializableElement(__desc, 36 + __off, Hoisted.subjectSer, it)
    }
    if (value.stage.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 37 + __off, Hoisted.stageSer, value.stage)
    if (value.evidence.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 38 + __off, Hoisted.evidenceSer, value.evidence)
