...
internal object TriggerDefinitionTimingSerializer : KSerializer<TriggerDefinition.Timing> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("TriggerDefinition.Timing") {
    ...
    encoder.encodeStructure(descriptor) {
      ...
      when (val __d = value) {
        ...
        is TriggerDefinition.Timing.Reference -> {
          encodeSerializableElement(__desc, 1, Hoisted.timingReferenceSer, __d.value)
        }
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): TriggerDefinition.Timing {
    val __desc = descriptor
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        1 ->
          timingReference =
            decoder.decodeNullableSerializableElement(__desc, 1, Hoisted.timingReferenceSer, null)
        2 -> timingDate = decoder.decodeStringElement(__desc, 2)
        3 ->
          _timingDate =
            decoder.decodeNullableSerializableElement(__desc, 3, Hoisted.elementSer, null)
        4 -> timingDateTime = decoder.decodeStringElement(__desc, 4)
        5 ->
          _timingDateTime =
            decoder.decodeNullableSerializableElement(__desc, 5, Hoisted.elementSer, null)
        CompositeDecoder.DECODE_DONE -> break
        else ->
          throw SerializationException("Unexpected index decoding TriggerDefinition.Timing: " + __i)
      }
    }
    return TriggerDefinition.Timing.from(
      timingTiming,
      timingReference,
