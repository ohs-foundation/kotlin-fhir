...
internal object PractitionerSerializer : KSerializer<Practitioner> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Practitioner {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        16 ->
          address = decoder.decodeNullableSerializableElement(__desc, 16, Hoisted.addressSer, null)
        17 -> gender = decoder.decodeStringElement(__desc, 17)
        18 ->
          _gender =
            decoder.decodeNullableSerializableElement(__desc, 18, Hoisted.implicitRulesSer, null)
        19 -> birthDate = decoder.decodeStringElement(__desc, 19)
        20 ->
          _birthDate =
            decoder.decodeNullableSerializableElement(__desc, 20, Hoisted.implicitRulesSer, null)
        21 -> photo = decoder.decodeNullableSerializableElement(__desc, 21, Hoisted.photoSer, null)
        22 ->
          qualification =
            decoder.decodeNullableSerializableElement(__desc, 22, Hoisted.qualificationSer, null)
        23 ->
          communication =
            decoder.decodeNullableSerializableElement(__desc, 23, Hoisted.communicationSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: Practitioner) {
    ...
    if (value.address.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 16, Hoisted.addressSer, value.address)
    ((value.gender?.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 17, it) }
    (value.gender?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 18, Hoisted.implicitRulesSer, it)
    }
    ((value.birthDate?.value?.toString()))?.let { encoder.encodeStringElement(__desc, 19, it) }
    (value.birthDate?.toElement())?.let {
      encoder.encodeSerializableElement(__desc, 20, Hoisted.implicitRulesSer, it)
    }
    if (value.photo.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 21, Hoisted.photoSer, value.photo)
    if (value.qualification.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 22, Hoisted.qualificationSer, value.qualification)
    if (value.communication.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 23, Hoisted.communicationSer, value.communication)
  }
