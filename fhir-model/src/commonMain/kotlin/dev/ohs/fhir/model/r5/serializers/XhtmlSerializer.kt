...
internal object XhtmlSerializer : KSerializer<Xhtml> {
  ...
  override fun serialize(encoder: Encoder, `value`: Xhtml) {
    encoder.encodeStructure(descriptor) {
      ...
      encodeSerializableElement(descriptor, 2, String.serializer(), value.`value`)
    }
  }

  override fun deserialize(decoder: Decoder): Xhtml =
    decoder.decodeStructure(descriptor) {
      ...
      while (true) {
        when (val index = decodeElementIndex(descriptor)) {
          ...
          2 -> `value` = decodeSerializableElement(descriptor, 2, String.serializer())
      ...
      Xhtml(id = id, extension = extension, `value` = `value`)
    }
}
