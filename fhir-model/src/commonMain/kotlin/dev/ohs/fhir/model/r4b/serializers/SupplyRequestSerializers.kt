...
internal object SupplyRequestItemSerializer : KSerializer<SupplyRequest.Item> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("SupplyRequest.Item") {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): SupplyRequest.Item {
    val __desc = descriptor
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        CompositeDecoder.DECODE_DONE -> break
        else -> throw SerializationException("Unexpected index decoding SupplyRequest.Item: " + __i)
      }
    }
    return SupplyRequest.Item.from(itemCodeableConcept, itemReference)!!
  }
