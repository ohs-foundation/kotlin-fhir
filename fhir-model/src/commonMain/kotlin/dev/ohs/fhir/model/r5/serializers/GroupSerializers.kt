...
internal object GroupCharacteristicValueSerializer : KSerializer<Group.Characteristic.Value> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Group.Characteristic.Value") {
      element("valueCodeableConcept", CodeableConcept.serializer().descriptor, isOptional = true)
      element("valueBoolean", KotlinBoolean.serializer().descriptor, isOptional = true)
      element("_valueBoolean", Element.serializer().descriptor, isOptional = true)
      element("valueQuantity", Quantity.serializer().descriptor, isOptional = true)
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Group.Characteristic.Value {
    val __desc = descriptor
    var valueCodeableConcept: CodeableConcept? = null
    var valueBoolean: KotlinBoolean? = null
    var _valueBoolean: Element? = null
      ...
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        CompositeDecoder.DECODE_DONE -> break
          ...
          throw SerializationException(
            "Unexpected index decoding Group.Characteristic.Value: " + __i
          )
      }
    }
    return Group.Characteristic.Value.from(
      valueCodeableConcept,
      R5Boolean.of(valueBoolean, _valueBoolean),
      valueQuantity,
      valueRange,
...
internal object GroupSerializer : KSerializer<Group> {
  override val descriptor: SerialDescriptor =
    buildClassSerialDescriptor("Group") {
      ...
      element("membership", KotlinString.serializer().descriptor, isOptional = true)
      element("_membership", Element.serializer().descriptor, isOptional = true)
      element("code", CodeableConcept.serializer().descriptor, isOptional = true)
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): Group {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        16 -> membership = decoder.decodeStringElement(__desc, 16)
        17 ->
          _membership =
        ...
        23 -> quantity = decoder.decodeIntElement(__desc, 23)
        24 ->
          _quantity =
            decoder.decodeNullableSerializableElement(__desc, 24, Hoisted.implicitRulesSer, null)
        25 ->
          managingEntity =
            decoder.decodeNullableSerializableElement(__desc, 25, Hoisted.managingEntitySer, null)
        26 ->
          characteristic =
            decoder.decodeNullableSerializableElement(__desc, 26, Hoisted.characteristicSer, null)
        27 ->
          member = decoder.decodeNullableSerializableElement(__desc, 27, Hoisted.memberSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: Group) {
    ...
    ((value.membership.value?.getCode()))?.let { encoder.encodeStringElement(__desc, 16, it) }
    ...
    ((value.quantity?.value))?.let { encoder.encodeIntElement(__desc, 23, it) }
    ...
    (value.managingEntity)?.let {
      encoder.encodeSerializableElement(__desc, 25, Hoisted.managingEntitySer, it)
    }
    if (value.characteristic.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 26, Hoisted.characteristicSer, value.characteristic)
    if (value.member.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 27, Hoisted.memberSer, value.member)
  }
