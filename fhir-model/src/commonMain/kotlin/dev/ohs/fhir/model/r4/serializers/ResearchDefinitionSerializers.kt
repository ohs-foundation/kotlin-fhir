...
internal object ResearchDefinitionSerializer : KSerializer<ResearchDefinition> {
  ...
  internal fun serializeJson(
    ...
    (value.population)?.let {
      encoder.encodeSerializableElement(__desc, 59 + __off, Hoisted.subjectReferenceSer, it)
    }
    (value.exposure)?.let {
      encoder.encodeSerializableElement(__desc, 60 + __off, Hoisted.subjectReferenceSer, it)
    }
    (value.exposureAlternative)?.let {
      encoder.encodeSerializableElement(__desc, 61 + __off, Hoisted.subjectReferenceSer, it)
    }
    (value.outcome)?.let {
      encoder.encodeSerializableElement(__desc, 62 + __off, Hoisted.subjectReferenceSer, it)
    }
  }
...
internal object ResearchDefinitionPolymorphicSerializer : KSerializer<ResearchDefinition> {
  ...
  override fun serialize(encoder: Encoder, `value`: ResearchDefinition) {
    encoder.encodeStructure(descriptor) {
      ResearchDefinitionSerializer.serializeJson(this, descriptor, 0, value)
    }
  }

  override fun deserialize(decoder: Decoder): ResearchDefinition =
    decoder.decodeStructure(descriptor) {
      ResearchDefinitionSerializer.deserializeJson(this, descriptor, 0)
    }
}
