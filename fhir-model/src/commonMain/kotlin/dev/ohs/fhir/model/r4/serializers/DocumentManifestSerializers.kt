  ...
  private fun deserializeJson(decoder: CompositeDecoder): DocumentManifest.Related {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        4 -> ref = decoder.decodeNullableSerializableElement(__desc, 4, Hoisted.refSer, null)
...
internal object DocumentManifestSerializer : KSerializer<DocumentManifest> {
  ...
  internal fun deserializeJson(decoder: CompositeDecoder): DocumentManifest {
    ...
    while (true) {
      when (val __i = decoder.decodeElementIndex(__desc)) {
        ...
        19 ->
          author = decoder.decodeNullableSerializableElement(__desc, 19, Hoisted.authorSer, null)
        20 ->
          recipient = decoder.decodeNullableSerializableElement(__desc, 20, Hoisted.authorSer, null)
        21 -> source = decoder.decodeStringElement(__desc, 21)
        22 ->
          _source =
        ...
        25 ->
          content = decoder.decodeNullableSerializableElement(__desc, 25, Hoisted.authorSer, null)
        26 ->
          related = decoder.decodeNullableSerializableElement(__desc, 26, Hoisted.relatedSer, null)
        CompositeDecoder.DECODE_DONE -> break
  ...
  private fun serializeJson(encoder: CompositeEncoder, `value`: DocumentManifest) {
    ...
    if (value.author.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 19, Hoisted.authorSer, value.author)
    if (value.recipient.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 20, Hoisted.authorSer, value.recipient)
    ((value.source?.value))?.let { encoder.encodeStringElement(__desc, 21, it) }
    ...
    if (value.content.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 25, Hoisted.authorSer, value.content)
    if (value.related.isNotEmpty())
      encoder.encodeSerializableElement(__desc, 26, Hoisted.relatedSer, value.related)
  }
